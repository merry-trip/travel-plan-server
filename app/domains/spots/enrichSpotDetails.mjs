...
export async function enrichSpotDetails(spot) {
  if (!spot.placeId) {
    logError(context, 'placeId is missing in spot');
    throw new Error('placeId is required for detail enrichment');
  }

  const url = `${BASE_URL}/${spot.placeId}?fields=${FIELDS}&key=${API_KEY}`;
  logInfo(context, `Request URL: ${url}`);

  try {
    const res = await fetch(url);
    const json = await res.json();

    logInfo(context, `Raw API response: ${JSON.stringify(json, null, 2)}`);

    if (!res.ok) {
      const errorMsg = json.error?.message || 'Unknown error';
      logError(context, `Places API error: ${errorMsg}`);
    ...
    logInfo(context, `Details enriched for ${spot.name || spot.placeId}`);
    return enriched;
  } catch (error) {
