const { getSheetClient } = require('./auth');
 ...
 */
async function updateSheetRow(sheetId, sheetName, placeId, updates = {}, columnOrder) {
  ...
  if (targetIndex === -1) {
    logger.logWarn('updateSheetRow', `行が見つかりませんでした: ${placeId}`);
    return null;
  }
  ...
  if (updated) {
    ...
    logger.logInfo('updateSheetRow', `✅ 更新成功: ${placeId} → ${JSON.stringify(updates)}`);
  } else {
    logger.logInfo('updateSheetRow', `🔁 更新不要（変更なし）: ${placeId}`);
  }

  return targetRow;
}

module.exports = updateSheetRow;
